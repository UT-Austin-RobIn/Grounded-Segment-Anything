    ...
    def detect(image):
        ...
        Returns:
            Dict: A dictionary containing all the relevant detection data
                ...
                "bbox_features" (np.ndarray)[256]: The embeddings of the focus object generated by DINO
        ...
        for focus_mask in range(detections.mask.shape[0]):
            ...
            for embedding_type in GroundedSAM.embedding_types:
                embeddings = GroundedSAM.get_embeddings(cropped_image, embedding_type)
                object_image_features[embedding_type].append(embeddings)
            ...
            if GroundedSAM.embedding_types.count("sam") > 0:
            ...
            part_masks.append(focus_mask_part_detections)
            ...
            focus_object_part_image_features = {}
            for embedding_type in GroundedSAM.embedding_types:
                focus_object_part_image_features[embedding_type] = []
            ...
            for i in range(focus_mask_part_detections.mask.shape[0]):
                ...
                for embedding_type in GroundedSAM.embedding_types:
                    embeddings = GroundedSAM.get_embeddings(cropped_part, embedding_type)
                    focus_object_part_image_features[embedding_type].append(embeddings)
        ...
        if(focus_mask is not None):
            ...
            out["object_masks"] = detections.mask
            out["label"] = [GroundedSAM.CLASSES[i] for i in detections.class_id]
            out["bbox_features"] = detections.data["features"]
...
def process_patch_points():
    ...
    for point in points:
        ...
        for i in range(len(GroundedSAM.detections[message["camera_name"]]["object_masks"])):
            if(GroundedSAM.detections[message["camera_name"]]["object_masks"][i][point[1], point[0]]):
                object_id = i
...
def dump_data(data, write_mode = None, camera = GroundedSAM.camera, directory = ""):
    ...
    if(write_mode != 1):
        ...
        for embedding_type in GroundedSAM.embedding_types:
            with open(data_directory + f"focus_patch_image_features_{embedding_type}.np", "wb") as f:
                np.save(f, data['focus_patch_image_features'][embedding_type])
    if(write_mode != 2):
        ...
        with open(data_directory + "object_masks.np", "wb") as f:
            np.save(f, data['object_masks'].astype(np.uint8))
        with open(data_directory + "bbox_features.np", "wb") as f:
            np.save(f, data['bbox_features'])
        with open(data_directory + "object_mask_features.np", "wb") as f:
            np.save(f, data['object_mask_features'])
...
def process_focus_point():
    ...
    for i in range(len(out["object_masks"])):
        if(out["object_masks"][i][focus_point[1], focus_point[0]]):
            out["focus_object"] = i
    ...
    if y > patch_image.shape[0] - 32:
        y = patch_image.shape[0] - 32
    patch_image[~out["object_masks"][out["focus_object"]]] = 0
    patch_image = patch_image[y - 32:y + 32, x - 32:x + 32]
    GroundedSAM.sam_predictor.set_image(patch_image)
    out["focus_patch_image_features"] = {}
    for embedding_type in GroundedSAM.embedding_types:
        out["focus_patch_image_features"][embedding_type] = GroundedSAM.get_embeddings(patch_image, embedding_type)
    dump_data(out, write_mode=2, camera=message["camera_name"], directory=data_directory)
...
if __name__ == '__main__':
    ...
    GroundedSAM.embedding_types_receiver = roslibpy.Topic(client, '/GroundedSAM/embedding_types', 'std_msgs/String')
    GroundedSAM.embedding_types_receiver.subscribe(embedding_types_callback)
